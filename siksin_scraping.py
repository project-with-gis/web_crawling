...
df = pd.read_csv('storeInfo_2.csv')
store_info = pd.DataFrame(df).loc[:]

if __name__ == '__main__':

    # 가게 추가 정보 크롤링과 리뷰 크롤링을 위한 인증값 크롤링
    store_df = add_siksin_info(store_info) #store_info.csv
    # print(store_df)
    # 리뷰 크롤링
    review_df = siksin_review_scraping(store_df)
    # print(review_df)

    path = './'
    # store_df.to_csv(os.path.join(path, 'siksin_store_info_add.csv'), header=False, index=False)
    review_df.to_csv(os.path.join(path, 'siksin_review_1030.csv'), encoding='UTF-8', header=False, index=False)
#
